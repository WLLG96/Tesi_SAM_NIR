...
class WindowAttention(nn.Module):
    ...
    def lepe_pos(self, v):
        ...
        lepe = self.get_v(v)
        lepe = lepe.view(B, NH, D, HW).permute(0, 1, 3, 2).contiguous()
        return lepe
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if MoE_config is None:
            ...
            print('-->>> MOE')
            print(MoE_config)
            self.mlp = MoE(input_size=dim, output_size=dim, hidden_size=mlp_hidden_dim, **MoE_config)
...
class Swin2MoSE(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
                 out_chans=1,  # ✅ NEW: controlla i canali di output
                 embed_dim=96, depths=[6, 6, 6, 6], num_heads=[6, 6, 6, 6],
                 window_size=7, mlp_ratio=4., qkv_bias=True,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, upscale=1, img_range=1., upsampler='', resi_connection='1conv',
                 use_lepe=False, use_cpb_bias=True, MoE_config=None, use_rpe_bias=False, **kwargs):
        super().__init__()

        num_in_ch = in_chans
        num_out_ch = out_chans  # ✅ QUI: output = out_chans
        ...
        # ✅ FIX: mean coerente con input channels
        if in_chans == 3:
